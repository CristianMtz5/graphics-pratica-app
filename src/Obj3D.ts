...
export class Obj3D {
  ...
  read(file: any): boolean {
    ...
    if (inp.fails()) return this.failing();
    this.file = file;
  ...
  readObject(inp: Input): boolean {
    ...
    for (;;) {
      ...
      if (vnrs.length >= 2) this.polyList.push(new Polygon3D(vnrs));
    }
    //inp.close();
  ...
  eyeAndScreen(dim: Dimension): number {
    ...
    let rangeX = xScrMax - xScrMin,
      rangeY = yScrMax - yScrMin;
    this.d = 0.95 * Math.min(dim.width / rangeX, dim.height / rangeY);
    this.imgCenter = new Point2D(
      (this.d * (xScrMin + xScrMax)) / 2,
  ...
  vp(cv: CvHLines, dTheta: number, dPhi: number, fRho: number): boolean {
    ...
    if (rhoNew >= this.rhoMin && rhoNew <= this.rhoMax) this.rho = rhoNew;
    else return false;
    cv.paint();
    return true;
  }
