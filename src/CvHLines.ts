...
export class CvHLines {
  ...
  buildLineSet(): void {
    ...
    this.nConnect = [];
    for (let i = 0; i < this.nVertices; i++) this.nConnect[i] = 0;
  ...
  lineSegment(
    ...
    let zP = p.z,
      zQ = q.z; // p and q give eye-coordinates
    ...
    for (let i = iStart; i < this.nTria; i++) {
      ...
      let iA = t.iA,
        iB = t.iB,
        iC = t.iC;
      let aScr: Point2D = vScr[iA],
        bScr: Point2D = vScr[iB],
        cScr: Point2D = vScr[iC];

      // 1. Minimax test for x and y screen coordinates:
      if (
        (maxPQx <= aScr.x && maxPQx <= bScr.x && maxPQx <= cScr.x) ||
      ...
      let zA = e[iA].z,
        zB = e[iB].z,
        zC = e[iC].z;
      if (minPQz >= zA && minPQz >= zB && minPQz >= zC) continue; // This triangle does not obscure PQ.
      ...
      if (
        (Tools2D.area2(aScr, bScr, pScr) < eps &&
      ...
      let a = pol.getA(),
        ...
        c = pol.getC(),
        h = pol.getH(),
        eps1 = 1e-5 * Math.abs(h),
        hP = a * p.x + b * p.y + c * p.z,
        hQ = a * q.x + b * q.y + c * q.z;
      if (hP > h - eps1 && hQ > h - eps1) continue; // This triangle does not obscure PQ.
      ...
      if (pInside && qInside) return; // This triangle obscures PQ.

      // 8. If P nearer than ABC and inside, PQ visible;
      //    the same for Q:
      let h1 = h + eps1;
      if ((hP > h1 && pInside) || (hQ > h1 && qInside)) continue; // This triangle does not obscure PQ.
      ...
      if (!pInside && lambdaMin > 0.001) {
        ...
        let zI = 1 / (lambdaMin / zQ + (1 - lambdaMin) / zP),
          xI = (-zI * iScrx) / d,
          yI = (-zI * iScry) / d;
        if (a * xI + b * yI + c * zI > h1) continue; // This triangle does not obscure PQ.
      ...
      if (!qInside && lambdaMax < 0.999) {
        ...
        let zJ = 1 / (lambdaMax / zQ + (1 - lambdaMax) / zP),
          xJ = (-zJ * jScrx) / d,
          yJ = (-zJ * jScry) / d;
        if (a * xJ + b * yJ + c * zJ > h1) continue; // This triangle does not obscure PQ.
        let jScr: Point2D = new Point2D(jScrx, jScry);
        if (Tools2D.distance2(jScr, qScr) >= 1.0)
          this.lineSegment(
            g,
            q,
            new Point3D(xJ, yJ, zJ),
            qScr,
            jScr,
            iQ,
            -1,
